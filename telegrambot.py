...
from information.idea_bounty import show_idea_bounty
...
async def handle_main_menu(update: Update, context: CallbackContext) -> None:
    ...
    if is_known_command(text):
        ...
        if text == "📉 Дебіторка (AR)":
            ...
            await show_idea_bounty(update, context)
...
async def handle_back_navigation(update: Update, context: CallbackContext) -> None:
    ...
    elif menu in ['currency_rates', 'user_guide', 'changelog', 'help_menu', 'devaluation_data', 'bug_bounty', 'idea_bounty']:
        await show_help_menu(update, context)
...
#   )
    ...
    scheduler.add_job(send_work_anniversary_greetings, 'cron', hour=8, minute=50, timezone='Europe/Kiev')
    ...
    app.add_handler(MessageHandler(filters.Regex("^(📉 Дебіторка (AR)|Назад|Таблиця|Гістограма|Діаграма|💼 Зарплата|💼 Оклад|🎁 Відомість Бонуси|ℹ️ Інформація|💱 Курс валют|Перевірка девальвації|Головне меню|📊 Аналітика|Аналітика за місяць|Аналітика за рік|2024|2025|2026|2027|2028|2029|2030|Січень|Лютий|Березень|Квітень|Травень|Червень|Липень|Серпень|Вересень|Жовтень|Листопад|Грудень|Дохід|Валовий прибуток|Маржинальність|Кількість угод|Протермінована дебіторська заборгованість|📘 Довідка|💰 Бонуси|👑 Премії керівників|🧾 Кадровий облік|🗓 Залишки|👔 Стаж|🕓 Відпрацьовано|📜 Відомість керівника|🧾 Опис змін|📊 Звіт В/Л|🐞 Bug Bounty|💡 Нові ідеї)$"), handle_main_menu))
