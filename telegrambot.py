...
async def handle_contact(update: Update, context: CallbackContext) -> None:
    if update.message.contact:
        ...
        if found:
            ...
            add_telegram_user(phone_number=phone_number, telegram_id=update.message.from_user.id, employee_name=employee_name)
...
async def handle_main_menu(update: Update, context: CallbackContext) -> None:
    ...
    if text == "Дебіторка":
        ...
        await handle_back_navigation(context, update)
...
async def handle_back_navigation(context, update):
...
def main():
    ...
    scheduler.add_job(check_new_payments, 'interval', minutes=1, max_instances=1, misfire_grace_time=60)
    scheduler.add_job(sync_payments, 'interval', minutes=1, max_instances=1, misfire_grace_time=60)
