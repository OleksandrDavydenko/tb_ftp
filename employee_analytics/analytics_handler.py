...
async def show_analytics_years(update: Update, context: CallbackContext) -> None:
    ...
    context.user_data['menu'] = 'analytics_years'  # Встановлюємо контекст для меню вибору року
    await update.message.reply_text("Оберіть рік:", reply_markup=reply_markup)

# Функція для відображення списку місяців для аналітики
async def show_analytics_months(update: Update, context: CallbackContext) -> None:
    ...
    # Створюємо кнопки з місяцями та додаємо кнопки "Назад" і "Головне меню"
    ...
    context.user_data['menu'] = 'analytics_months'  # Встановлюємо контекст для меню вибору місяця
    await update.message.reply_text("Оберіть місяць:", reply_markup=reply_markup)

# Функція для відображення даних аналітики за вибраний період
