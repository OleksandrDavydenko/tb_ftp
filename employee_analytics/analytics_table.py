...
# Функція для отримання даних про дохід менеджера або сейлза за рік і місяць
def get_income_data(employee_name, role, year, month):
    ...
    # Словник для перетворення українських місяців у формат, який містить місяць і рік
    months_uk = {
    ...
    month_name = months_uk.get(month, None)
    if not month_name:
        logging.error(f"Неправильний місяць: {month}")
        return None

    # Формуємо текстовий формат дати для порівняння з колонкою 'RegistrDate'
    formatted_date = f"{month_name} {year} р."

    # Вибір колонки для фільтрування залежно від ролі
    role_column = "Manager" if role == "Менеджер" else "Seller"

    # Запит для отримання доходу з фільтром на ім'я та відформатовану дату
    query_data = {
        "queries": [
            {
                "query": f"""
